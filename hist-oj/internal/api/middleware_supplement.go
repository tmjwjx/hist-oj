...
// AdminOrProblemAdminAuthMiddleware 管理员权限验证中间件（root/admin/problem_admin）
func AdminOrProblemAdminAuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		logger := utils.GetLogger()
		...
		isAllowed := false
		for _, role := range roleList {
			if role == "root" || role == "admin" || role == "problem_admin" {
				isAllowed = true
				break
			}
		}
		...
		if !isAllowed {
			logger.Warn("用户不是管理员或题目管理员", zap.Any("roles", roleList))
			c.JSON(200, errorResponse(403, "需要管理员或题目管理员权限"))
			c.Abort()
			return
		}
