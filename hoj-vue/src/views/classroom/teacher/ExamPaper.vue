...
export default {
  ...
  watch: {
    showCreateDialog(newVal, oldVal) {
      console.log('showCreateDialog changed from', oldVal, 'to', newVal, 'isEditMode:', this.isEditMode)
      ...
      if (oldVal === true && newVal === false) {
        ...
        console.log('Form reset complete in watch')
      }
    },
    showProblemDetailDialog(newVal, oldVal) {
      console.log(`showProblemDetailDialog changed from ${oldVal} to ${newVal}`)
      if (newVal === true) {
        console.log('Dialog should be opening now')

        // 检查 data 中的值
        console.log('Current data state:', {
          showProblemDetailDialog: this.showProblemDetailDialog,
          currentViewProblem: this.currentViewProblem,
          fetchingViewProblem: this.fetchingViewProblem
        })
        ...
        this.$nextTick(() => {
          setTimeout(() => {
            ...
            console.log(`Dialog wrappers count after opening: ${dialogWrappers.length}`)
            dialogWrappers.forEach((wrapper, i) => {
              const dialog = wrapper.querySelector('.el-dialog')
              const title = dialog ? dialog.querySelector('.el-dialog__title') : null
              console.log(`Wrapper ${i}: title="${title ? title.textContent : 'null'}"`)
            })
            ...
            if (!problemDetailDialogExists) {
              console.error('Programming problem detail dialog was not created!')
  ...
  methods: {
    openCreateDialog() {
      ...
      console.log('Before reset - isEditMode:', this.isEditMode, 'isEditing:', this.isEditing, 'title:', this.paperForm.title)
    ...
    doOpenCreateDialog() {
      console.log('========== doOpenCreateDialog called ==========')
      // 取消任何正在进行的编辑操作
      this.isEditing = false
      ...
      console.log('After reset - isEditMode:', this.isEditMode, 'isEditing:', this.isEditing, 'title:', this.paperForm.title)
      ...
      this.$nextTick(() => {
        ...
        console.log('Dialog opened, isEditMode:', this.isEditMode)
      })
    },
    canEditPaper(paper) {
    ...
    async editPaper(row) {
      ...
      try {
        ...
        // 再次检查是否已取消编辑（双重检查）
        if (!this.isEditing) {
          console.log('Edit operation was cancelled, ignoring response')
          return
        }
    ...
    async fetchAndviewProblemDetail(problemId) {
      console.log('fetchAndviewProblemDetail called with problemId:', problemId)
      if (!problemId) {
      ...
      console.log('Dialog should be visible now:', this.showProblemDetailDialog)
      ...
      // 延迟检查 DOM
      this.$nextTick(() => {
        console.log('In nextTick, dialog visible:', this.showProblemDetailDialog)

        // 强制设置编程题详情对话框的 z-index
        setTimeout(() => {
          const dialogWrappers = document.querySelectorAll('.el-dialog__wrapper')
          console.log('Found dialog wrappers:', dialogWrappers.length)
          ...
          if (dialogWrappers.length >= 2) {
            ...
            dialogWrappers.forEach((wrapper, i) => {
              ...
              console.log(`Dialog wrapper ${i}: title="${titleText}"`)

              if (titleText === '编程题详情') {
                problemDetailWrapper = wrapper
                problemDetailDialog = dialog
                console.log('Found problem detail dialog at index', i)
              }
            })

            // 如果没找到，使用最后一个作为后备
            if (!problemDetailWrapper) {
              console.log('Problem detail dialog not found by title, using last wrapper')
            ...
            if (problemDetailWrapper && problemDetailDialog) {
              console.log('Setting z-index for problem detail dialog')
              console.log('Wrapper classList:', problemDetailWrapper.classList)
              console.log('Dialog classList:', problemDetailDialog.classList)
            ...
            console.log('Setting z-index for modals, count:', modals.length)
            modals.forEach(m => {
            ...
            // 验证设置是否成功
            setTimeout(() => {
              console.log('After setting z-index:')
              console.log('Wrapper computed z-index:', window.getComputedStyle(problemDetailWrapper).zIndex)
              if (problemDetailDialog) {
                console.log('Dialog computed z-index:', window.getComputedStyle(problemDetailDialog).zIndex)
              }

              // 检查 wrapper 的位置和可见性
              const wrapperRect = problemDetailWrapper.getBoundingClientRect()
              const wrapperStyles = window.getComputedStyle(problemDetailWrapper)
              console.log('Wrapper info:', {
                ...
                rect: wrapperRect,
                inViewport: wrapperRect.top >= 0 && wrapperRect.left >= 0 &&
              ...
              // 检查对话框内容
              if (problemDetailDialog) {
                const dialogContent = problemDetailDialog.querySelector('.problem-detail-view')
                const dialogHeader = problemDetailDialog.querySelector('.el-dialog__header')
                const dialogBody = problemDetailDialog.querySelector('.el-dialog__body')
                console.log('Dialog content check:', {
                  hasDetailView: !!dialogContent,
                  hasHeader: !!dialogHeader,
                  hasBody: !!dialogBody,
                  bodyHTML: dialogBody ? dialogBody.innerHTML.substring(0, 200) : 'null'
                })
              }
            }, 50)
            }
          ...
          console.log('All el-dialog elements:', dialogs.length)
          dialogs.forEach((d, i) => {
          ...
          // 检查 v-modal 遮罩层
          const modals = document.querySelectorAll('.v-modal')
          console.log('v-modal count:', modals.length)
          modals.forEach((m, i) => {
            console.log(`v-modal ${i} z-index:`, window.getComputedStyle(m).zIndex)
          })
        }, 100)
      })
      ...
      try {
        ...
        if (res && res.status === 200 && res.data && res.data.data && res.data.data.problem) {
          ...
          console.log('Problem data loaded:', this.currentViewProblem)
        } else {
        ...
        console.log('fetchingViewProblem set to false')
      }
    },
